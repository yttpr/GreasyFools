...
// Type: GreasyFools.ResourceLoader
    ...
    public static Sprite LoadSprite(string name, int ppu = 32, Vector2? pivot = null)
    {
      ...
      try
      {
        ...
        using (MemoryStream memoryStream = new MemoryStream())
        {
          ...
          sprite = Sprite.Create(texture, new Rect(0.0f, 0.0f, (float) texture.width, (float) texture.height), pivot.Value, (float) ppu);
        }
      }
      catch (InvalidOperationException ex)
      {
        ...
        return ResourceLoader.LoadSprite("EVILPASSIVEICON.png");
      }
      return sprite;
    }
